...
<div class="card">
  <div class="card-body">
    <div class="d-flex align-items-center mb-4">
      ...
      <div>
        <h5 class="mb-1">{{ trim(($user->firstName ?? '') . ' ' . ($user->middleName ?? '') . ' ' . ($user->lastName ?? '')) ?: 'Tenant' }}</h5>
        <small class="text-muted">{{ $user->email ?? 'No email on file' }}</small>
      </div>
    </div>

    <form method="POST" action="{{ route('tenants.profile.update') }}">
      @csrf
      @method('PUT')
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="contactNo" class="form-control" value="{{ old('contactNo', $user->contactNo) }}" placeholder="09XXXXXXXXX" required>
        ...
        <div class="col-12">
          ...
          <input type="text" name="homeAddress" class="form-control" value="{{ old('homeAddress', $user->homeAddress) }}" required>
